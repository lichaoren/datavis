...
public class ViewController3D {
	...
	enum Mode {
		NONE, ROTATE, ROTATE_Z, PAN, ZOOM, PUSH
	};
	...
	void move(float mx, float my) {
		...
		case PAN: {
			double d[] = { -0.3 * pCurr.d * (mCur[0] - mOld[0]),
					-0.3 * pCurr.d * (mCur[1] - mOld[1]), 0 };
	...
	void home(double ex, double ey, double ez, double cx, double cy, double cz,
		...
		pCurr.d = Math.sqrt(look[0] * look[0] + look[1] * look[1] + look[2]
				* look[2]);
		...
		double ls = Math.sqrt(side[0] * side[0] + side[1] * side[1] + side[2]
				* side[2]);
		...
		double tq[] = { 1 + m[0][0] + m[1][1] - look[2],
	...
	void apply_trackball(double[] m0, double[] m1) {
		...
		double sp1[] = {
				m0[0],
				m0[1],
				d1 < tbsize ? Math.sqrt(2.0 * tbsize - d1) : tbsize
						/ Math.sqrt(d1) };
		double sp2[] = {
				m1[0],
				m1[1],
				d2 < tbsize ? Math.sqrt(2.0 * tbsize - d2) : tbsize
						/ Math.sqrt(d2) };

		double axis[] = { sp2[1] * sp1[2] - sp2[2] * sp1[1],
		...
		double angle = Math.sqrt(sp2[0] * sp2[0] + sp2[1] * sp2[1] + sp2[2]
				* sp2[2])
				/ tbsize;
	...
	void update() {
		mvMatrix[0] = 1.0 - 2.0 * pCurr.r[1] * pCurr.r[1] - 2.0 * pCurr.r[2]
				* pCurr.r[2];
		mvMatrix[1] = 2.0 * pCurr.r[0] * pCurr.r[1] - 2.0 * pCurr.r[2]
				* pCurr.r[3];
		mvMatrix[2] = 2.0 * pCurr.r[0] * pCurr.r[2] + 2.0 * pCurr.r[1]
				* pCurr.r[3];
		mvMatrix[3] = 0;

		mvMatrix[4] = 2.0 * pCurr.r[0] * pCurr.r[1] + 2.0 * pCurr.r[2]
				* pCurr.r[3];
		mvMatrix[5] = 1.0 - 2.0 * pCurr.r[0] * pCurr.r[0] - 2.0 * pCurr.r[2]
				* pCurr.r[2];
		mvMatrix[6] = 2.0 * pCurr.r[1] * pCurr.r[2] - 2.0 * pCurr.r[0]
				* pCurr.r[3];
		mvMatrix[7] = 0;

		mvMatrix[8] = 2.0 * pCurr.r[0] * pCurr.r[2] - 2.0 * pCurr.r[1]
				* pCurr.r[3];
		mvMatrix[9] = 2.0 * pCurr.r[1] * pCurr.r[2] + 2.0 * pCurr.r[0]
				* pCurr.r[3];
		mvMatrix[10] = 1.0 - 2.0 * pCurr.r[0] * pCurr.r[0] - 2.0 * pCurr.r[1]
				* pCurr.r[1];
		mvMatrix[11] = 0;

		mvMatrix[12] = -(mvMatrix[0] * pCurr.c[0] + mvMatrix[4] * pCurr.c[1] + mvMatrix[8]
				* pCurr.c[2]);
		mvMatrix[13] = -(mvMatrix[1] * pCurr.c[0] + mvMatrix[5] * pCurr.c[1] + mvMatrix[9]
				* pCurr.c[2]);
		mvMatrix[14] = -(mvMatrix[2] * pCurr.c[0] + mvMatrix[6] * pCurr.c[1]
				+ mvMatrix[10] * pCurr.c[2] + pCurr.d);
		mvMatrix[15] = 1;

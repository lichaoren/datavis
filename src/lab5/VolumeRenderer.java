...
public class VolumeRenderer
{
  class ShaderProgram
  {
    ...
    {
      if( vsrcfile != null )
        this.vsrc = PApplet.join( parent.loadStrings( vsrcfile ), '\n' );
        
      if( fsrcfile != null )
        this.fsrc = PApplet.join( parent.loadStrings( fsrcfile ), '\n' );
    }
    ...
    {  
      ...
      int ival[] = { 0 };
      gl.glGetShaderiv( shader, GL2ES2.GL_COMPILE_STATUS, ival, 0 );
      ...
      if( !compiled )
      {
        gl.glGetShaderiv( shader, GL2ES2.GL_SHADER_TYPE, ival, 0 );
        int type = ival[0];
    
        String prefix = "unknown";
    
        if( type == GL2ES2.GL_FRAGMENT_SHADER )
          PApplet.print( "fragment" );
        else if( type == GL2ES2.GL_VERTEX_SHADER )
          PApplet.print( "vertex" );
      
        PApplet.println( " shader failed to compile --------" );
  
        // check for and print info log
        gl.glGetShaderiv( shader, GL2ES2.GL_INFO_LOG_LENGTH, ival, 0 );
        ...
        if( ival[0] > 0 )
        {
          ...
          PApplet.println( new String(bInfoLog) );
        }
    ...
    void enable( GL2 gl )
    {
      if( pid == 0 )
      {
        ...
        vid = gl.glCreateShader( GL2ES2.GL_VERTEX_SHADER );
        gl.glAttachShader( pid, vid );
    
        fid = gl.glCreateShader( GL2ES2.GL_FRAGMENT_SHADER );
    ...
    boolean linkProgram( GL2 gl, int program )
    {
      ...
      int ival[] = { 0 };    
      gl.glGetProgramiv( program, GL2ES2.GL_LINK_STATUS, ival, 0 );
   
      boolean linked = (ival[0] == GL.GL_TRUE);
  
      if( !linked )
      {
        PApplet.println( "GLSL program failed to link --------" );
  
        // check for and print info log
        gl.glGetProgramiv( program, GL2ES2.GL_INFO_LOG_LENGTH, ival, 0 );
        if( ival[0] > 0 )
        {
          byte bInfoLog[] = new byte[ival[0]];
  ...
  void bindVolumeTexture( GL2 gl )
  {
    ...
    if( data != null )
    {
      ...
      gl.glBindTexture( GL2ES2.GL_TEXTURE_3D, volumeTex );
      gl.glTexImage3D( GL2ES2.GL_TEXTURE_3D, 0, GL.GL_LUMINANCE, vsize, vsize, vsize, 0, GL.GL_LUMINANCE, GL.GL_UNSIGNED_BYTE, buffer );
  ...
  void renderCube( GL2 gl, int mode )
  {
    ...
    if( mode == GL2GL3.GL_LINE )
    {
      gl.glPolygonMode( GL.GL_FRONT_AND_BACK, GL2GL3.GL_LINE );
      
      // gl.glDrawElements( gl.GL_LINES, 24, gl.GL_UNSIGNED_INT, edgeIndexBuffer );      
      gl.glDrawElements( GL2GL3.GL_QUADS, 24, GL.GL_UNSIGNED_INT, faceIndexBuffer );
...
//  }    
  ...
  void draw()
  {
    ...
    if( glversion == null )
    {
      ...
      glversion += "GLSL version:   " + gl.glGetString( GL2ES2.GL_SHADING_LANGUAGE_VERSION ) + '\n';
      
      PApplet.println( glversion );
    }
